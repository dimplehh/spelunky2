...
void CTileMgr::SetDecoLand(HDC hDC, int _iScrollX, int _iScrollY)
{
	...
	for (int _index = 0; _index < m_vecTile.size(); _index++)
	{
		...
		if (!m_vecTile.empty())
		{
			if (2 < _x && m_vecTile[_index]->Get_Option() == 1 && m_vecTile[_index - 1]->Get_Option() == 0) //왼쪽 벽
			{
			...
			if (_x < TILEX - 1 && m_vecTile[_index]->Get_Option() == 1 && m_vecTile[_index + 1]->Get_Option() == 0) //오른쪽 벽
			{
			...
			if (1 < _y && m_vecTile[_index]->Get_Option() == 1 && m_vecTile[_index - TILEX]->Get_Option() == 0) //밟는 땅
			{
			...
			if (_y < TILEY - 2 && m_vecTile[_index]->Get_Option() == 1 && m_vecTile[_index + TILEX]->Get_Option() == 0) // 아래땅
			{
