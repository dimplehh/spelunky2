...
void CStage::Initialize()
{
	...
	CLineMgr::Get_Instance()->Set_Box_Line(TILECX * (59 + 0.5f), TILECY * 10);

	CObjMgr::Get_Instance()->Add_Object(OBJ_BOX, CAbstractFactory<CBox>::Create(TILECX * (38 + 0.5f), TILECY * 17));
	CLineMgr::Get_Instance()->Set_Box_Line(TILECX * (38 + 0.5f), TILECY * 17);

	CObjMgr::Get_Instance()->Add_Object(OBJ_BOX, CAbstractFactory<CBox>::Create(TILECX * (20 + 0.5f), TILECY * 6));
	CLineMgr::Get_Instance()->Set_Box_Line(TILECX * (20 + 0.5f), TILECY * 6);

	CObjMgr::Get_Instance()->Add_Object(OBJ_BOX, CAbstractFactory<CBox>::Create(TILECX * (15 + 0.5f), TILECY * 1));
	CLineMgr::Get_Instance()->Set_Box_Line(TILECX * (15 + 0.5f), TILECY * 1);
